package fi.fcode.gui;

import fi.fcode.SectorsPlugin;
import fi.fcode.gui.api.GuiWindow;
import fi.fcode.gui.api.builder.ItemBuilder;
import fi.fcode.helpers.ChatHelper;
import fi.fcode.helpers.CustomHeadHelper;
...
public class ChannelGui {
    ...
    public static void openGui(Player player) {
        ...
        for (Sector sector : SectorsPlugin.getInstance().getSectorCache().getSectorMap().values()) {
            if (sector.getSectorType() == SectorType.SPAWN) {
                ...
                guiWindow.setItem(i, item.buildFromItemStack(), (event -> {
                    ...
                    SectorsPlugin.getInstance().getUserCache().getUser(player.getUniqueId()).ifPresent(user -> {
